...
alone \citep{baxter2011sociotechnical}.
...
an undifferentiated transfer of authority \citep{amershi2019guidelines}.
...
\citep{w3c2013provdm,w3c2025vcdm}. What remains less clearly specified is
how traceability, accountability, contestability, and evidence boundaries
should be preserved when bounded local entities configure modular
institutional functions under higher-order constitutional constraints.
...
platform, functioning multi-tenant infrastructure, users, evaluation
results, validated controls, or operational authority.
...
one inspectable decision structure. Participation readiness, internal
maturity distinctions, and risk boundaries remain supporting components that
frame how this structure might later be evaluated, reduced, or rejected.
...
layer may preserve coherence, while Tenant-level governance still requires
bounded discretion over local decisions and configurations.
\item \textbf{Modularity versus governability.} Service separation may limit
direct coupling, while modular selection can create hidden dependency chains,
...
\noindent\textbf{Illustrative synthetic scenario - Tenant module exception
review.} A mock Tenant governance body requests a temporary exception
affecting one service nucleus. The request includes the applicable
...
applied, which local choice was proposed, what the simulated AI review
...
\item Tenant governance interprets that scope for a bounded local decision
context and proposes a service-nucleus configuration or exception;
\item service nuclei express the bounded institutional capability being
selected, reviewed, or changed;
...
non-delegable constraints visible before local configuration begins. In this
model, a valid action requires both institutional justification and whatever
technical authorization a future implementation might use; neither alone
would establish legitimacy.
...
prescribe a storage system, cryptographic proof, or integrity guarantee.
...
analysis support, adversarial checking, and operational planning support.
...
claim that AI assistance itself enforces governance or security.
