# Evidence Boundaries

Status: Phase 1 editorial control

## Publicly allowed

- Axodus is presented as a conceptual, prototype-stage infrastructure model.
- The project originates from a multi-year independent research and design
  effort.
- Proof of Knowledge is proposed as an educational participation mechanism.
- L0–L5 and D0–D5 are internal operational maturity dimensions for,
  respectively, organizational/governance maturity and
  development/implementation maturity. They are not empirically validated
  measurement instruments and do not confer production status or sensitive
  authority.
- Governance, human oversight, accountability, risk boundaries, and modularity
  are design objectives and research topics.
- Proposed evaluation methods and falsifiable research questions may be
  described as future work.

## Blocked until evidence or explicit human approval

- a precise project duration;
- a precise number, identity, or completion state of cores;
- operational readiness or production use;
- implementation completeness;
- adoption, users, partners, traction, or impact;
- performance, safety, security, or reliability guarantees;
- treasury, token, financial-performance, or valuation claims;
- affiliations, endorsements, grants, or institutional relationships;
- comparative superiority or novelty claims.

## Required distinctions

- **Externally supported fact**: supported by an appropriate public source.
- **Internal claim**: supplied by an author but not publicly evidenced.
- **Hypothesis**: a testable expectation.
- **Design proposal**: a normative or architectural choice not yet validated.
- **Future work**: an intended activity, not a completed result.

Blocked claims stay in `research/claims-register.md` and do not enter a public
manuscript. Editorial convenience is not grounds to relax this boundary.
